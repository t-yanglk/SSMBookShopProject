...
<body>
	...
	<form class="select formcss" action="commodity/selectcomm" method="post">
	<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<input type="text" name="commprice" lay-verify="required"
					placeholder="请输入价格进行查询" autocomplete="off" class="layui-input">
			</div>
			<div class="layui-input-inline">
				<input type="text" name="preid" lay-verify="required"
					placeholder="请输入父类ID进行查询" autocomplete="off" class="layui-input">
			</div>
			<button type="submit" class="layui-btn">查询商品</button>
			<button type="button" class="layui-btn renovatecomm">刷新表格</button>
		</div>
	</form>
			<form class="formcss addcomm" action="commodity/addcommjsp" method="post">
				<button type="button" class="layui-btn addcheck layui-btn-normal" ><i class="layui-icon"></i></button>
			</form>
			<form class="formcss updatecomm" action="commodity/updatecommjsp" method="post">  
				<button type="button" class="layui-btn updatecheck"><i class="layui-icon"></i></button>
			</form>
				<button type="button" class="layui-btn delcheckcomm layui-btn-danger"><i class="layui-icon"></i></button>
		...
		<table class="layui-table">
			...
			<tbody>
				<c:forEach items="${comms }" var="item">
					<tr>
						<td><input class="check" type="checkbox" lay-skin="primary"></td>
						<td class="select">${item.commid }</td>
						<td>${item.commname }</td>
						<td>￥${item.commprice }</td>
						<td>${item.commimg }</td>
						<td class="rootid">${item.rootid }</td>
						<td class="preid">${item.preid }</td>
						<td>${item.description }</td>
						<td>${item.count }</td>
						<td>
							<form class="formcss" action="commodity/updatecommjsp" method="post">
								<input type="hidden" name="commid" value="${item.commid }"/>
								<button type="submit" class="layui-btn layui-btn-sm update">
									<i class="layui-icon"></i>
								</button>
							</form>
							<button type="button" class="layui-btn layui-btn-sm layui-btn-danger deletecomm">
								<i class="layui-icon"></i>
							</button>
						</td>
					</tr>
		...
		<form class="form" action="commodity/selectcomm" method="post"
			style="display: none">
